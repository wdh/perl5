...
S_force_word(pTHX_ char *start, int token, int check_keyword, int allow_pack)
{
	...
	if (check_keyword) {
	  char *s2 = PL_tokenbuf;
	  STRLEN len2 = len;
	  if (allow_pack && memFOO_STARTING_WITHs(s2, len, "CORE::"))
	    s2 += sizeof("CORE::") - 1, len2 -= sizeof("CORE::") - 1;
...
Perl_yylex(pTHX)
{
  ...
  retry:
	...
	do {
		...
		/* Incest with pod. */
                if (   *s == '='
                    && memFOO_STARTING_WITHs(s, PL_bufend - s, "=cut")
                    && !isALPHA(s[4]))
                {
                    SvPVCLEAR(PL_linestr);
		    PL_oldoldbufptr = PL_oldbufptr = s = PL_linestart = SvPVX(PL_linestr);
	...
	case XATTRTERM:
	 ...
	 grabattrs:
		...
		if (PL_lex_stuff) {
		    ...
		    else if (!PL_in_my && memEQs(SvPVX(sv), len, "lvalue")) {
			sv_free(sv);
			CvLVALUE_on(PL_compcv);
		    }
		    else if (!PL_in_my && memEQs(SvPVX(sv), len, "locked")) {
			sv_free(sv);
                        deprecate_disappears_in("5.28",
	...
	case KEY_foreach:
		...
		char *p = s;
                ...
                if (   memFOO_STARTING_WITHs(p, PL_bufend - p, "my")
                    && isSPACE(*(p + 2)))
                {
                    p += 2;
                }
                else if (   memFOO_STARTING_WITHs(p, PL_bufend - p, "our")
                         && isSPACE(*(p + 3)))
                {
                    p += 3;
                }
